...
for (mod in unique_modifications) {
    ...
    g <- g + geom_smooth(formula = y ~ splines::ns(x, 4, intercept = TRUE), alpha=0.5)
...
for (mod in unique_modifications) {
  ...
  g <- ggplot(
    ...
    geom_smooth(formula = y ~ splines::ns(x, 4, intercept = TRUE), alpha=0.5) +
    facet_wrap(~ RawFile)
  g <- g + ggtitle(mod)
  print(g)
}
...
df <- df %>%
  group_by(RawFile) %>% 
  mutate(
    ResidualRT = (RetentionTime - predict(lm(formula = RetentionTime ~ splines::ns(
      Prediction_irt, df=4, intercept = TRUE)))))
...
tmp_df <- df %>% 
  ...
  ungroup() %>%
  select(Sample = RTError, Prediction = ResidualRT)

label_data <- lapply(tmp_df, function(x) {mean(abs(x))})
title_string <- paste(
  paste0("MeanAbsError ", names(label_data)),
...
title_string <- paste0("Median PredResidualRT: ", format(median(df$ResidualRT), digits = 3))
