...
for (mod in unique_modifications) {
    g <- ggplot(df, aes(x = RetentionTime, y = Prediction_irt, colour = grepl(mod, Peptide))) + 
      geom_point(shape = ".")  +
    facet_wrap(~ RawFile)
    g <- g + ggtitle(mod)
    print(g)
...
for (mod in unique_modifications) {
    ...
    g <- ggplot(
      ...
      xlim(-20, 20)+ ggtitle(mod)
    
    print(g)
    
    g <- ggplot(
      df, aes(y = ResidualRT, fill = grepl(mod, Peptide))) +
      geom_boxplot() + ggtitle(mod)
    ...
    print(g)
     ...
     g <- ggplot(
      ...
      geom_violin(draw_quantiles = c(0.5), alpha=0.3) + ggtitle(mod) + coord_cartesian(ylim=c(0, 10))
...
for (mod in unique_modifications) {
    ...
    g <- tmp_df2 %>%
        ...
        geom_text(x = median(1 -tmp_df2$Loss_loss_angle), y = 50, label=median(1 -tmp_df2$Loss_loss_angle), inherit.aes = FALSE) +
        xlim(0,1) + coord_cartesian(expand=FALSE)
