...
```{r}
my_lm <- mgcv::gam(RetentionTime ~ Prediction_irt, data=df %>% group_by(Peptide) %>% top_n(1, `mokapot score`))


df$residual_irt <- predict(my_lm, newdata = df) - df$RetentionTime
...
for (mod in unique_modifications) {
    ...
    g <- ggplot(df, aes(x = residual_irt, fill = grepl(mod, Peptide))) +
